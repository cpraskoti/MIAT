...
class GDEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.lstm_encoder_size = args['lstm_encoder_size']  # will be used as transformer model dim
        ...
        # Instead of LSTM, we add a projection to the desired transformer dimension
        self.input_proj = nn.Linear(self.traj_linear_hidden, self.lstm_encoder_size)
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.lstm_encoder_size,
                                                    nhead=self.n_head,
    ...
    def forward(self, hist, nbrs, mask, va, nbrsva, lane, nbrslane, cls, nbrscls):
        ...
        # self agent: apply linear projection and activation, then project to transformer dim
        ...
        # Permute to (batch, seq_len, d_model) for later processing
        hist_hidden_enc = hist_hidden_enc.permute(1, 0, 2)
        # nbrs agent: similar processing, but leave output in (seq_len, batch, d_model) shape
        nbrs_enc = self.activation(self.linear1(nbrs))
        nbrs_enc_proj = self.input_proj(nbrs_enc)
        nbrs_hidden_enc = self.transformer(nbrs_enc_proj)
        ...
        # Split query for multi-head attention
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        # Replace LSTM with a Transformer block for decoding.
    ...
    def forward(self, dec, lat_enc, lon_enc):
        ...
        # Apply the transformer decoder (which expects input of shape [seq_len, batch, d_model])
